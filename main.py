...
def add_educat(request:schemas.Educational,db:Session=Depends(get_db)):
...
def educational(db:Session=Depends(get_db)):
...
def updateedu(sno,emp_id, request: schemas.updateedu,db:Session=Depends(get_db)):
...
def personal(access_key:str,db:Session=Depends(get_db)):
...
def team(reporting_to_emp_id:str,db:Session=Depends(get_db)):
...
def auto(report_to_emp_id,db:Session=Depends(get_db)):
...
def updatpersonal(emp_id, request: schemas.personalup,db:Session=Depends(get_db)):
...
def fina(db:Session=Depends(get_db)):
...
def l(emp_id:str,db:Session=Depends(get_db)):
...
@app.get('/pay',tags=['payslip dates'])
def pays(emp_id:str,db:Session=Depends(get_db)):
    payslip=db.query(models.Eps.my).where(models.Eps.empid==emp_id).order_by(models.Eps.sno)[::-1]
    return payslip
